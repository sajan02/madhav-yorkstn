...
const services = [
  ...
  { n: "04", name: "Store Design, Buildout & Brand Fidelity", tag: "Retail Experience", desc: "We project-manage the entire store fit-out against your brand guidelines — sourcing vendors who can meet specification, overseeing materials and fixtures, managing lighting design, and ensuring every element reflects your identity precisely.", items: ["Brand guideline interpretation for Indian buildout", "Vendor identification & quality management", "Fixtures, lighting & materials sourcing", "Visual merchandising & layout", "Soft launch event management", "Post-launch brand compliance audits"] },
  ...
  return (
    <>
      ...
      <section style={{ padding: '96px 60px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 72, marginBottom: 64, alignItems: 'end' }}>
          <h2 className="rv" style={{ fontFamily: 'var(--fd)', fontSize: 'clamp(34px,4.5vw,58px)', fontWeight: 300, lineHeight: 1.05, color: 'var(--b)' }}>
            Why brands struggle in India —<br />and <em style={{ fontStyle: 'italic', color: 'var(--bl)' }}>how we fix it</em>
          </h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 2, background: 'rgba(55,24,37,.08)' }}>
          {problems.map((p, i) => (
            <article key={p.n} className="pc rv" style={{ background: 'var(--c)', padding: '40px 34px', position: 'relative', overflow: 'hidden' }}>
              ...
              <p style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid rgba(55,24,37,.1)', fontSize: 10, color: 'var(--bl)', display: 'flex', gap: 8, lineHeight: 1.7 }}>
                <span style={{ color: 'var(--g)', flexShrink: 0 }}>→</span>{p.s}
              </p>
            </article>
          ))}
        </div>
            ...
            return (
              <div key={s.n} style={{ borderTop: '1px solid rgba(55,24,37,.1)' }}>
                <button onClick={() => setOpenIdx(isOpen ? null : i)}
                  ...
                  <span style={{ width: 32, height: 32, border: '1px solid rgba(55,24,37,.18)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, background: isOpen ? 'var(--b)' : 'none', color: isOpen ? 'var(--c)' : 'var(--b)', transform: isOpen ? 'rotate(45deg)' : 'none', transition: 'all .4s', flexShrink: 0 }}>+</span>
                </button>
                {isOpen && (
                  <div style={{ paddingLeft: 72, paddingBottom: 36 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1.2fr .8fr', gap: 44 }}>
                      <p style={{ fontSize: 12.5, lineHeight: 2.1, color: 'var(--tx)', fontWeight: 300 }}>{s.desc}</p>
                      <div>
                        <p style={{ fontSize: 8, letterSpacing: '.28em', textTransform: 'uppercase', color: 'var(--g)', marginBottom: 14, fontWeight: 400 }}>What you receive</p>
                        <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 8 }}>
                          {s.items.map((item) => (
                            <li key={item} style={{ fontSize: 11, color: 'var(--tx)', fontWeight: 300, display: 'flex', gap: 9, lineHeight: 1.65 }}>
                              <span style={{ color: 'var(--g)', opacity: .6, fontSize: 8.5, flexShrink: 0, marginTop: 2 }}>—</span>{item}
                            </li>
                          ))}
                        </ul>
                      </div>
      ...
      {/* CTA */}
      <section style={{ padding: '72px 60px', textAlign: 'center', background: 'var(--c)' }}>
        <span className="ey rv" style={{ justifyContent: 'center', display: 'inline-flex', marginBottom: 16 }}>Get Started</span>
        <h2 className="rv d1" style={{ fontFamily: 'var(--fd)', fontSize: 'clamp(26px,4vw,50px)', fontWeight: 300, color: 'var(--b)', marginBottom: 24, marginTop: 18, lineHeight: 1.1 }}>
          Not sure which services you need?<br /><em style={{ fontStyle: 'italic', color: 'var(--bl)' }}>Tell us about your brand.</em>
        </h2>
        <button className="btn btn-d rv d2" onClick={openInquiry}>Submit Brand Enquiry →</button>
      </section>
